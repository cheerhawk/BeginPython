...
class SquishSprite(pygame.sprite.Sprite):
    ...
    by the screen size and margin.
    """
    def __init__(self, image):
        super().__init__()
        self.image = pygame.image.load(image).convert()
        self.rect = self.image.get_rect()
        sceen = pygame.display.get_surface()
        shrink = -config.margin * 2
        self.area = sceen.get_rect().inflate(shrink, shrink)
...
class Banana(SquishSprite):
    ...
    with tis horizontal position governed by the current mouse
    ...
    def __init__(self):
        ...
        # consittue a hit (or , rather, a squish):
        self.pad_top = config.banana_pad_top
        self.pad_size = config.banana_pad_side

    def update(self):
        """
        Set the Banana's center x-coordinate to the current mouse
        x-coordinate, and then use the rect method clamp to ensure
        that the Banana stays within tis allowed range of motion.
        """
    ...
    def touches(self, other):
        ...
        new rectangle is first calulated (using the rect method
        inflate with the side and top paddings) that dow not include
        the 'empty' areas on the top and sides ot the banana.
        """
        # Defalte the bouds with the proper padding:
