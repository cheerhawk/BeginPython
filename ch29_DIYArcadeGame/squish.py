...
class Level(State):
    ...
    def display(self, screen):
        """
        Display the state after the first display (which simply wipes the screen).
        As opposed to firstDisplay, this method uses
...
class Paused(State):
    ...
    def handle(self, event):
        """
        Hanles events by delegating to State (which handles quitting
        in general) and by reacting to key presses and mouse
        clicks. If a key is pessed or the mouse is cliecked,
    ...
    def first_display(self, screen):
        ...
        """
        # First, clear the screen by filling it with the backgroud color:
        screen.fill(config.backgroud_color)

        # Create a Font object with the default appearance, and specitfied size:
        ...
        # Calculate the placement of the text (centerd on the screen):
        ...
        if self.image:
            ...
            # get its rect:
            r = image.get_rect()
            # move the text down by half th image height:
            top += r.height // 2
            # place the image 20 pixels above the text:
            r.midbottom = center, top - 20
            # blit the image to the sceen:
            screen.blit(image, r)
...
class Game:
    """
    A game object that takes care of the main event loop,
    including changing between the different game states.
    """
    ...
    def run(self):
        ...
        # Decicde whether to display the game in a window or to use the
        # full screen:
        flag = 0            # Default (window) mode

        if config.full_screen:
            flag = FULLSCREEN       # Full screen mode
        sceen_size = config.screen_size
        sceen = pygame.display.set_mode(sceen_size, flag)
        ...
        while True:
            ...
            if self.state != self.next_state:
                ...
                self.state.first_display(sceen)
